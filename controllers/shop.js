 ...
 *
 * NEW IN THIS VERSION: restricted-product-type email verification. For any
 * purchased product_type in RESTRICTED_PRODUCT_TYPES (currently just
 * "SquareAcrylicSign"), the checkout email MUST match a record in the
 * GiftEmail collection (i.e. must be a registered account's email). If it
 * doesn't, the order is automatically refunded via Stripe and no route
 * assignment / sold-count / confirmation email happens for that order.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ─── NEW: Restricted product types requiring a registered checkout email ────
// Any purchased line item whose product_type appears here forces a check:
// the Stripe checkout email MUST exist in the GiftEmail collection, or the
// entire order is refunded and cancelled.
const RESTRICTED_PRODUCT_TYPES = new Set(['SquareAcrylicSign']);

/**
 * enforceRegisteredEmailRestriction
 *
 * Returns true  -> order is valid, proceed with normal fulfillment.
 * Returns false -> order was refunded; caller must skip fulfillment and
 *                  mark ProcessedOrder as cancelled.
 */
async function enforceRegisteredEmailRestriction({ customerEmail, purchasedItems, paymentIntentId, sessionId }) {
  ...
  if (!hasRestrictedItem) return true; // nothing restricted in this cart — always valid
  ...
  if (!customerEmail) {
    ...
    if (giftRecord) return true; // email matches a registered account — valid
  }

  // ── Invalid: refund the payment ─────────────────────────────────────────────
...
async function assignRoutesForPurchase(email, purchasedItems, sessionId = null) {
  ...
  for (const item of purchasedItems) {
    ...
    for (let i = 0; i < quantity; i++) {
      try {
        const claimedRoute = await QRRedirect.findOneAndUpdate(
          { product_type: productType, ownerId: null, active: true },
          { $set: { ownerId } },
          { new: true }
        );
        ...
        if (claimedRoute) {
          ...
          console.log(`✅  [Shop] Assigned route "${claimedRoute.route}" (${productType}) to ${normalizedEmail} (ownerId: ${ownerId})`);
        } else {
...
export async function createCheckoutSession(req, res) {
  ...
  try {
    ...
    const dbProducts = await Product.find({ slug: { $in: slugs }, inStock: true })
      .select('slug title priceUSD images product_type')
      .lean();
    ...
    for (const item of items) {
      ...
      if (dbProduct.product_type) {
        purchasedByType[dbProduct.product_type] =
          (purchasedByType[dbProduct.product_type] || 0) + (item.quantity || 1);
      }
    }
 ...
 *
 * NEW FLOW:
 *   1. If already processed AND previously marked cancelled -> immediately
 *      return status: 'cancelled_refunded' (handles page reloads correctly).
 *   2. If already processed (not cancelled) -> skip straight to building the
 *      normal paid receipt response (existing behavior).
 *   3. If NOT already processed -> claim it, then:
 *        a. Run enforceRegisteredEmailRestriction(). If it fails, refund,
 *           mark ProcessedOrder.cancelled = true, and return
 *           status: 'cancelled_refunded' — skip sold counts, route
 *           assignment, and the normal confirmation email entirely.
 *        b. If it passes, run the existing fulfillment: sold counts,
 *           route assignment + order logging, confirmation email.
 */
export async function verifySession(req, res) {
  ...
  try {
    ...
    // Repeat call for an order we already cancelled — report that consistently.
    ...
    if (!alreadyProcessed) {
      ...
      if (!emailCheckPassed) {
        ...
        // ── Normal fulfillment (unchanged from before) ──────────────────────
