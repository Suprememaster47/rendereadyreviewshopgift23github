 ...
 *
 * CRITICAL FIELD: ownerId. This must always be set to user._id — it's the
 * ONLY thing that lets gifting and Stripe route-assignment work without
 * ever touching the users collection. Uses $set (not $setOnInsert) on
 * ownerId/displayName/profilePic so this also repairs any pre-existing
 * GiftEmail doc that's missing ownerId.
 *
 * Logs loudly on both success and failure — if this ever silently stops
 * working again, your console will show either the confirmation line or
 * the full Mongoose error instead of nothing at all.
 */
async function ensureGiftEmailRecord(user) {
  ...
  try {
    ...
    console.error(err); // full stack — helps diagnose validation/duplicate key errors
  }
}
...
exports.postSignup = async (req, res, next) => {
  ...
  try {
    ...
    // Every newly registered account gets a GiftEmail record with ownerId
    // set to the real user._id — this is the ONLY place gifting/route
    // assignment needs to look, never the users collection.
    console.log(`[signup] User saved (${user._id}) — now syncing GiftEmail for ${user.email}...`);
...
exports.postUpdateProfile = async (req, res, next) => {
  ...
  try {
    ...
    // Keep GiftEmail in sync — repeat this call every profile save (not
    // just on email change) so ownerId/displayName/profilePic never drift.
    await ensureGiftEmailRecord(user);
