...
// ─── Fetch auth status (for nav dropdown profile pic) ────────────────────────
...
// "Store" inside the dropdown navigates within the SPA (pushState) rather
// than doing a full page reload, since /shop/collections and /shop share
// this same document. Every other dropdown link (Home, Account, Login,
// Sign Up) is a real server route and uses a normal <a href> — no JS needed.
...
/**
 * Categories are now derived directly from whatever `category` values exist
 * on the loaded products — no hardcoded list. Add a new category value to
 * any product doc in Atlas and it appears here automatically on next load.
 * Falls back to just "All" if products haven't loaded or have no category set.
 */
